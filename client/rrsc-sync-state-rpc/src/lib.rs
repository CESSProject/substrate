...
//!
//! Currently only usable with RRSC + GRANDPA.
//!
//! # Usage
//!
	...
	Client: HeaderBackend<Block> + sc_client_api::AuxStore + 'static,
{
	...
	fn build_sync_state(&self) -> Result<LightSyncState<Block>, Error<Block>> {
		...
		Ok(LightSyncState {
			...
			rrsc_epoch_changes: self.shared_epoch_changes.shared_data().clone(),
			rrsc_finalized_block_weight: finalized_block_weight,
