...
pub struct RRSCConsensusDataProvider<B: BlockT, C> {
	...
	/// RRSC config, gotten from the runtime.
	config: Config,

	/// Authorities to be used for this rrsc chain.
	authorities: Vec<(AuthorityId, RRSCAuthorityWeight)>,
}

/// Verifier to be used for rrsc chains
pub struct RRSCVerifier<B: BlockT, C> {
	/// Shared epoch changes
	epoch_changes: SharedEpochChanges<B, Epoch>,
...
impl<B: BlockT, C> RRSCVerifier<B, C> {
	/// create a nrew verifier
	pub fn new(epoch_changes: SharedEpochChanges<B, Epoch>, client: Arc<C>) -> RRSCVerifier<B, C> {
		RRSCVerifier { epoch_changes, client }
	}
}

/// The verifier for the manual seal engine; instantly finalizes.
#[async_trait::async_trait]
impl<B, C> Verifier<B> for RRSCVerifier<B, C>
where
	B: BlockT,
...
impl<B, C> RRSCConsensusDataProvider<B, C>
where
	B: BlockT,
	C: AuxStore
		+ HeaderBackend<B>
...
impl<B, C> ConsensusDataProvider<B> for RRSCConsensusDataProvider<B, C>
where
	B: BlockT,
	C: AuxStore
		+ HeaderBackend<B>
