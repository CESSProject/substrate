...
use cessc_consensus_rrsc::{authorship, Epoch};
...
use cessp_consensus_rrsc::{
	digests::PreDigest, AuthorityId, RRSCApi as RRSCRuntimeApi, RRSCConfiguration,
};
...
mod tests {
	...
	use cessc_consensus_rrsc::{block_import, AuthorityPair};
	use std::sync::Arc;
	...
	fn test_rrsc_rpc_module(
		...
		RRSC::new(client.clone(), epoch_changes, keystore, config, longest_chain, deny_unsafe)
	}
