...
const SLOT_DURATION_MS: u64 = 6000;
...
impl WithRuntime for RRSCTestNet {
	fn with_runtime(rt_handle: Handle) -> Self {
		RRSCTestNet { rt_handle, peers: Vec::new() }
	}
...
fn rejects_empty_block() {
	...
	let mut net = RRSCTestNet::new(runtime.handle().clone(), 3);
...
fn run_one_test(mutator: impl Fn(&mut TestHeader, Stage) + Send + Sync + 'static) {
	...
	let runtime = Runtime::new().unwrap();
	let net = RRSCTestNet::new(runtime.handle().clone(), 3);
...
fn can_author_block() {
	...
	let mut config = crate::RRSCConfiguration {
		slot_duration: 1000,
		epoch_length: 100,
		c: (3, 10),
		authorities: Vec::new(),
...
fn propose_and_import_block<Transaction: Send + 'static>(
	...
	import
		.insert_intermediate(INTERMEDIATE_KEY, RRSCIntermediate::<TestBlock> { epoch_descriptor });
...
fn importing_block_one_sets_genesis_epoch() {
	let runtime = Runtime::new().unwrap();
	let mut net = RRSCTestNet::new(runtime.handle().clone(), 1);
