...
pub const RRSC_VRF_PREFIX: &[u8] = b"substrate-rrsc-vrf";

/// RRSC VRFInOut context.
pub static RRSC_VRF_INOUT_CONTEXT: &[u8] = b"RRSCVRFInOutContext";

/// A RRSC authority keypair. Necessarily equivalent to the schnorrkel public key used in
/// the main RRSC module. If that ever changes, then this must, too.
#[cfg(feature = "std")]
pub type AuthorityPair = app::Pair;

/// A RRSC authority signature.
pub type AuthoritySignature = app::Signature;

/// A RRSC authority identifier. Necessarily equivalent to the schnorrkel public key used in
/// the main RRSC module. If that ever changes, then this must, too.
...
pub type RRSCAuthorityWeight = u64;

/// The cumulative weight of a RRSC block, i.e. sum of block weights starting
...
pub fn make_transcript(randomness: &Randomness, epoch: u64) -> Transcript {
...
#[cfg(feature = "std")]
pub fn make_transcript_data(randomness: &Randomness, epoch: u64) -> VRFTranscriptData {
	VRFTranscriptData {
	...
	H: Header,
{
	...
	let verify_seal_signature = |mut header: H, offender: &AuthorityId| {
		let seal = header.digest_mut().pop()?.as_rrsc_seal()?;
