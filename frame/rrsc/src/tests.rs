...
use cessp_consensus_rrsc::{AllowedSlots, RRSCEpochConfiguration, Slot};
...
fn first_block_epoch_zero_start() {
	...
	ext.execute_with(|| {
		...
		let consensus_log = cessp_consensus_rrsc::ConsensusLog::NextEpochData(
			cessp_consensus_rrsc::digests::NextEpochDescriptor {
				authorities: RRSC::authorities().to_vec(),
...
fn current_slot_is_processed_on_initialization() {
	...
	ext.execute_with(|| {
		...
		System::reset_events();
		System::initialize(&1, &Default::default(), &pre_digest);
		assert_eq!(RRSC::current_slot(), Slot::from(0));
		assert!(RRSC::initialized().is_none());

		// current slot is updated on initialization
		RRSC::initialize(1);
		assert_eq!(RRSC::current_slot(), genesis_slot);
		assert!(RRSC::initialized().is_some());
...
fn can_enact_next_config() {
	new_test_ext(1).execute_with(|| {
		...
		let current_config = RRSCEpochConfiguration {
			c: (0, 4),
			allowed_slots: cessp_consensus_rrsc::AllowedSlots::PrimarySlots,
		};
		...
		let consensus_log =
			cessp_consensus_rrsc::ConsensusLog::NextConfigData(NextConfigDescriptor::V1 {
				c: next_next_config.c,
				allowed_slots: next_next_config.allowed_slots,
			});
		let consensus_digest = DigestItem::Consensus(RRSC_ENGINE_ID, consensus_log.encode());
...
fn only_root_can_enact_config_change() {
	...
	new_test_ext(1).execute_with(|| {
		...
		let res = RRSC::plan_config_change(RuntimeOrigin::none(), next_config.clone());

		assert_noop!(res, DispatchError::BadOrigin);

		let res = RRSC::plan_config_change(RuntimeOrigin::signed(1), next_config.clone());

		assert_noop!(res, DispatchError::BadOrigin);

		let res = RRSC::plan_config_change(RuntimeOrigin::root(), next_config);
...
fn report_equivocation_current_session_works() {
	...
	ext.execute_with(|| {
		...
		let key = (cessp_consensus_rrsc::KEY_TYPE, &offending_authority_pair.public());
...
fn report_equivocation_invalid_key_owner_proof() {
	...
	ext.execute_with(|| {
		...
		let key = (cessp_consensus_rrsc::KEY_TYPE, &authorities[1].0);
		let key_owner_proof = Historical::prove(key).unwrap();
...
fn valid_equivocation_reports_dont_pay_fees() {
	...
	ext.execute_with(|| {
		...
		// create the key ownership proof.
		let key_owner_proof =
			Historical::prove((cessp_consensus_rrsc::KEY_TYPE, &offending_authority_pair.public()))
				.unwrap();
